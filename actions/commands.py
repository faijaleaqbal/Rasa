...
def _dispatch_slash_command(
    ...
    # 37. /pdf <title> [content]
        ...
        pdf_content = f"Generated by @Alya_Rasa_Bot for user {user_id}.\n\nDocument Title: {title}\nCreated: {os.uname().nodename}"
        fpath, msg = docs.create_pdf_file(title, pdf_content)
    ...
    # 76. /youtube <url> or /yt <url>
        ...
        return {"handled": True, "text": content_skills.summarize_youtube_video(args_str)}

    # 77. /summarize <url> or /article <url>
    elif cmd in ["/summarize", "/article", "/webpage"]:
        return {"handled": True, "text": content_skills.summarize_webpage(args_str)}

    # 78. /briefing or /morning
    elif cmd in ["/briefing", "/morning"]:
        return {"handled": True, "text": content_skills.get_daily_briefing(user_id, args_str or "Malda")}
